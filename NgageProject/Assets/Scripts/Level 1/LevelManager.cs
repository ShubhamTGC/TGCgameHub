...
public class LevelManager : MonoBehaviour
{
    ...
    public void spawnLeft() //to pool a level part at left side
    {
        if (!spawnedLeft)
        {
            ...
            newLevelObj.transform.position = new Vector3(this.transform.position.x - 191.8f, transform.position.y, transform.position.z);
    ...
    public void enableEnemies()
    {
        if (canSpawnEnemies)
        {
            ...
            {
                enemyList[i].transform.position = new Vector3(Random.Range(tempPosX + 25, tempPosX + 50), -4.93f , 0); //this will prevent overlapping of enemies

...
public class Level2Manager : MonoBehaviour
{
    ...
    public void spawnLeft() //to pool a level part at left side
    {
        if (!spawnedLeft)
        {
            GameObject newLevelObj = Spawn2Manager.instant.getPooledLevel();
            ...
            spawnedLeft = true;
            newLevelObj.GetComponent<Level2Manager>().spawnedRight = true; //to check if there is any level part on the right side
            newLevelObj.GetComponent<Level2Manager>().coinSpawned = true; //this will prevent activating coins of left side
            newLevelObj.GetComponent<Level2Manager>().canSpawnEnemies = false; //this will prevent activating enemies at left side
        }
    ...
    public void spawnRight()
    {
        if (!spawnedRight)
        {
            ...
            newLevelObj.SetActive(true);
            spawnedRight = true;
            newLevelObj.GetComponent<Level2Manager>().spawnedLeft = true; //to check if there is any level part on left side
            newLevelObj.GetComponent<Level2Manager>().coinSpawned = false; // this will indicate that we can activate coins on that level part
            newLevelObj.GetComponent<Level2Manager>().canSpawnEnemies = true; // this will indicate that we can actictivate enemies on that level part
        }
    ...
    public void deactiveOther() //deactivating other level parts
    {
        ...
        {
            Spawn2Manager.instant.removeOthers();
            otherDc = true;
        }
    }
    ...
    public void enableEnemies()
    {
        if (canSpawnEnemies)
        {
            ...
            {
                enemyList[i].transform.position = new Vector3(Random.Range(tempPosX + 25, tempPosX + 50), -4.45f, 0); //this will prevent overlapping of enemies

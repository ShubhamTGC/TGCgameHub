...
public class Spawn3Manager : MonoBehaviour
{
    ...
    public static Spawn3Manager instant;
    ...
    public void removeOthers() //disabling all level parts other than current level part
    {
        for (int i = 0; i < pooledLevelParts.Count; i++)
        {
            if (!pooledLevelParts[i].GetComponent<Level3Manager>().isCurrent)
            {
                pooledLevelParts[i].SetActive(false);
                pooledLevelParts[i].GetComponent<Level3Manager>().coinSpawned = false; //this will be reset the coin spawn boolean
                pooledLevelParts[i].GetComponent<Level3Manager>().disbleCoins(); //disabling coins which are active in their poollist
                pooledLevelParts[i].GetComponent<Level3Manager>().disableEnemies(); // disabling all the enemies on the level part
                pooledLevelParts[i].GetComponent<Level3Manager>().disablePlatforms();
            }
        }
    }
    ...
    {
        gameManager.isReplaying[2] = true;
        Time.timeScale = 1;
    ...
    IEnumerator getAttemptNo()
    {
        ...
        string cred = "?UID=" + gameManager.ID_USER.ToString() + "&GID=7&RID=8";
